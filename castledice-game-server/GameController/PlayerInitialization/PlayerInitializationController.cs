...
public class PlayerInitializationController : IPlayerInitializationController
{
    ...
    public async Task InitializePlayerAsync(string playerToken, ushort clientId)
    {
        try
        {
            ...
            _logger.Info($"Initializing player with id {playerId} and client id {clientId}");
            if (_idsMap.PlayerHasClient(playerId))
            {
                _logger.Info($"Player with id {playerId} already has a client id assigned. Disconnecting the old client.");
                _playerDisconnecter.DisconnectPlayerWithId(playerId);
            }
            _idsMap.SaveClientToPlayer(playerId, clientId);
            _logger.Info($"Player with id {playerId} successfully initialized with client id {clientId}");
            _logger.Info($"Sending initialization result to client with id {clientId}");
