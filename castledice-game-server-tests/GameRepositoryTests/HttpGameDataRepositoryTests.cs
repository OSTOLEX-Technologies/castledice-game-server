...
public class HttpGameDataRepositoryTests
{
    ...
    [InlineData("http://some_site/")]
    [InlineData("http://some_site/with/some/path")]
    public async void PutGameDataAsync_ShouldSendMessageWithAppropriateUrl(string url)
    {
        var httpMessageSenderMock = new Mock<IHttpMessageSender>();
        SetUpPutResponse(httpMessageSenderMock, GetGameData());
        var httpGameDataRepository = new HttpGameDataRepository(url, httpMessageSenderMock.Object);
        Predicate<HttpRequestMessage> requestHasProperUrl = request =>
            request.RequestUri.ToString() == url + "/game/" + GetGameData().Id;
        
        await httpGameDataRepository.PutGameDataAsync(GetGameData());
    ...
    [Theory]
    [MemberData(nameof(GameDataCases))]
    public async void PutGameDataAsync_ShouldSendMessageWithSerializedGameData(GameData expectedData)
    {
        var httpMessageSender = new TestMessageSender();
        var response = new HttpResponseMessage
        {
            StatusCode = HttpStatusCode.OK,
            Content = new StringContent(JsonConvert.SerializeObject(expectedData))
        };
        httpMessageSender.ResponseMessage = response;
        var httpGameDataRepository = new HttpGameDataRepository("http://localhost:5000", httpMessageSender);
        
        await httpGameDataRepository.PutGameDataAsync(expectedData);
        
        Assert.True(httpMessageSender.ContentString == JsonConvert.SerializeObject(expectedData));
    }

    [Fact]
    public async void PutGameDataAsync_ShouldSendMessageWithAppropriateMediaType()
    {
        var expectedMediaType = "application/json";
        var httpMessageSenderMock = new Mock<IHttpMessageSender>();
        SetUpPutResponse(httpMessageSenderMock, GetGameData());
    ...
    [Theory]
    [MemberData(nameof(GameDataCases))]
    public async void PutGameDataAsync_ShouldReturnGameData_DeserializedFromResponse(GameData expectedGameData)
    {
        var httpMessageSenderMock = new Mock<IHttpMessageSender>();
        SetUpPutResponse(httpMessageSenderMock, expectedGameData);
        var httpGameDataRepository = new HttpGameDataRepository("http://localhost:5000", httpMessageSenderMock.Object);
        
        var actualGameData = await httpGameDataRepository.PutGameDataAsync(GetGameData());
        
        Assert.Equal(expectedGameData, actualGameData);
    }
    
    [Fact]
    public async void PutGameDataAsync_ShouldThrowInvalidOperationException_IfCannotConvertResponseToGameData()
    {
        var response = new HttpResponseMessage
        {
            StatusCode = HttpStatusCode.OK,
        ...
        await Assert.ThrowsAsync<InvalidOperationException>(async () => await httpGameDataRepository.PutGameDataAsync(GetGameData()));
    }
    ...
    private static void SetUpPutResponse(Mock<IHttpMessageSender> mockMessageSender, GameData expectedGameData)
    {
        var jsonToReturn = JsonConvert.SerializeObject(expectedGameData);
        var response = new HttpResponseMessage
        {
